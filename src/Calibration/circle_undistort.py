...
objp[:, :2] = np.indices(tate, yoko).T.reshape(-1, 2)
...
mtx = np.array([563.68967506, 0, 292.97169075, 0, 565.55027553, 279.85442658, 0, 0, 1]).reshape(3,3)
#dist = np.array(["""***************ここにあらかじめ求めておいた歪み係数を書く***************"""])
dist = np.array([1.24326168, -8.65370547, 0.02621078, -0.02648232, 19.91564754])
...
while True:
    ...
    if ret1:
        ...
        ret2, corners =  cv2.findCirclesGrid(gray, (tate, yoko), flags=cv2.CALIB_CB_SYMMETRIC_GRID)
        print(ret2)
